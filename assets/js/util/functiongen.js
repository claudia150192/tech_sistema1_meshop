...
jQuery.fn.SelectAjax = function(datasource,all)
{	
	if( typeof datasource == "undefined")
		$(this).each(function()
		{
			...
			if(typeof data.aaData == "undefined")
				cargarSelect(data,$(this).attr("id"),$(this).attr("attrval"),$(this).attr("attrdesc"),all);
			else
				cargarSelect(data.aaData,$(this).attr("id"),$(this).attr("attrval"),$(this).attr("attrdesc"),all);
		});
	else
	{
		if(typeof datasource.aaData == "undefined")
			cargarSelect(datasource,$(this).attr("id"),$(this).attr("attrval"),$(this).attr("attrdesc"),all);
		else
			cargarSelect(datasource.aaData,$(this).attr("id"),$(this).attr("attrval"),$(this).attr("attrdesc"),all);
	}
...
function createDataTable2(idTable, options){
	...
	var oTable = $('#'+idTable).dataTable({
		...
		"fnInitComplete": TOptios.fnInitComplete,
	 	...
	 	"oTableTools": {
       	"sSwfPath": "/meshop/assets/swf/copy_csv_xls_pdf.swf",
           "aButtons": [
           "csv",
           "xls",
           "pdf"
...
function reloadclosemodal(idmodal,idaTable){
	var returnfunction = function(data){
		...
		//$('#'+idaTable).dataTable().ajax.reload();
		console.log(data);
		$('#'+idmodal).modal('hide');
		$('#'+idmodal+" form").reset();
		};
...
function cargarDep(idselect, ubigeo){
	...
	$(ubigeo).each(function(){
		if(this.int_departamento == 1)
			result += '<option value="'+this.int_id+'">'+this.var_descripcion+'</option>';
	});
	$select.html(result); 
}
...
function cargarProv(idselect, ubigeo, iddepend){
	...
	var result = '';
	$(ubigeo).each(function(){
		if(this.int_provincia == 1 && this.int_dependencia == iddepend)
			result += '<option value="'+this.int_id+'">'+this.var_descripcion+'</option>';
	});
	$select.html(result); 
}
...
function cargarDist(idselect, ubigeo, iddepend){
	...
	var result = '';
	$(ubigeo).each(function(){
		if(this.int_distrito == 1 && this.int_dependencia == iddepend)
			result += '<option value="'+this.int_id+'">'+this.var_descripcion+'</option>';
	});
	$select.html(result); 
}
...
function cargarUbigeo(ubigeos,idtagdist, idtagprov, idtagdep, iddist){
	...
	if(typeof(iddist) != 'undefined')
	{
		...
		$(ubigeos).each(function(){
			if(this.int_id == dist.int_dependencia)
			{
				prov = this;
				return 0;
			}
	...
	if(prov!= null){
		$('#'+idtagdep).val(prov.int_dependencia);
	}

	cargarProv(idtagprov, ubigeos, $('#'+idtagdep).val());

	if(prov!= null){
		$('#'+idtagprov).val(prov.int_id);
	}
...
function cargarSelect(arreglo, idselect, attrvalue, attrdescripcion,all){
	var $select = $('#'+idselect);
	var result = '';
	
	if(typeof all != "undefined")
		if (all)
			result += '<option value="0">Todos</option>';		
...
function fechaFirstYear(){
	date = new Date();
	Fecha = "01/01/"+date.getFullYear();
	return Fecha;
}
...
Prefijo
*/
var Jerarquia = function (elemento) {
        var res = 0;
        switch (elemento) {
        case ')':
            res = 5; break;
...
var isopdor = function (elemento)
{
    if(String(elemento).search("[\*|\+|\/|\^|\)|\(|\ ]") == -1 && String(elemento).search("-") == -1)
        return false;
    else
        return true;
}

var strToInfijo = function (expinfijo){
    var arrayinfijo = [];
    for(var i = 0; i < expinfijo.length; i++)
    {
        if(expinfijo[i] != " ")
        {
            var character = expinfijo[i];
            while (i+1 < expinfijo.length && !isopdor(character+expinfijo[i+1]))
            {
                i++;
                character+= expinfijo[i];
            }
            arrayinfijo.push(character);
        }           
    }
    return arrayinfijo;
}

var replaceValue = function(expinfijo, values)
{
    $(expinfijo).each(function(index){
        if(typeof values[this] != "undefined" )
            expinfijo[index] = values[this];
        else
        {
        	expinfijo[index] = String(this);
        }
    });
    return expinfijo;
}

var prefijo = function (expinfijo, values){
	var arrayinfijo = replaceValue(strToInfijo(expinfijo),values);
	var caracter;
    var PilaDefinitiva = [], PilaTemp = [];
	while(caracter = arrayinfijo.shift())
	{
		switch(caracter)
		{
            case '(':
            ...
            case '+':case '-':case '^':case '*':case '/':
                while (Jerarquia(caracter) <= Jerarquia(PilaTemp[PilaTemp.length-1]))
                    PilaDefinitiva.push(PilaTemp.pop());
                PilaTemp.push(caracter);
                break;
            case ')':
                while (PilaTemp[PilaTemp.length-1] != '(')
                {
                    PilaDefinitiva.push(PilaTemp.pop());
                }                    
                PilaTemp.pop()
...
var calcular = function(expresion)
{
    var pila = [];
    while(expresion.length > 0)
    {
        var element = expresion.shift();
        if(!isopdor(element))
            pila.push(element);
        else
        {
            var opdoA = parseFloat(pila.pop());
            var opdoB =  parseFloat(pila.pop());
            var result = 0;
            switch(element)
            {
                case "-":
